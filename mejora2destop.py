#si la media del loss de una epoca es mas grande que la de la anterior, ya no mejoramos asi que paramos y ya está
...
valdataset = TensorDataset(conjuntovalimages, conjuntoval_labels)
test_dataset = TensorDataset(conjuntotest, conjuntotest_labels)

#fijar la semilla en cada cosa aleatoria que faig
#usar la tecnica de emilio para el seed
...
while(sigo):
    ...
    epoch+=1
    if mejor<(sum2 / len(val_loader)):
        if epoch - mejor_epoch>minparar:
            sigo=False

    else:
        mejor_epoch=epoch
        mejor=(sum2 / len(val_loader))
